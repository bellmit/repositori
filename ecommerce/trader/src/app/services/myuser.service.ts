...
export class MyuserService {
  ...
  postLogin(user: MyUser): Observable<MyUser> {
	alert('http://localhost:8083/' + this.myUserUrl+'/login');
      return this.http.post<MyUser>('http://localhost:8083' + this.myUserUrl+'/login', user, this._reqOptionsArgs).pipe(map(res => {
