...
export class MockServer {
	  ...
	  initialLoad(): Observable<any> {
 		...
 		var promesa  = new Promise((resolve, _reject) => {
			...
			let urlConnexio = 'http://localhost:8887/enviaments/';
			alert(urlConnexio);
			httpRequest.open('GET',	urlConnexio);     // 8083 = reactive
			httpRequest.send();

			httpRequest.onreadystatechange = () => {
				//alert("334 " + httpRequest.readyState + "  " + httpRequest.status);
	...
	/* Observable de la carrega nom�s de les files modificades */
	 ...
	 setBidAndAsk(dataItem: { enviamentId: string; comandaId: string; estat: string; }) {
		//alert(dataItem.enviamentId);
